---
title: "Corncob_DA_Analysis"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Corncob_DA_Analysis}
...
```{r eval=FALSE, fig.fullwidth=T,Run_CornCob_Tests}
...
## test height while controlling for sex...

sex_height_g <- corncob::differentialTest(formula = ~ Extraction_Number + A_SDC_GENDER + PM_STANDING_HEIGHT_AVG,
                          formula_null = ~ Extraction_Number + A_SDC_GENDER,
                          phi.formula = ~ Extraction_Number + A_SDC_GENDER + PM_STANDING_HEIGHT_AVG,
                          phi.formula_null = ~ Extraction_Number + A_SDC_GENDER + PM_STANDING_HEIGHT_AVG,
                          data=phylo_comp_g, test="Wald", fdr_cutoff = 0.1, boot=F)

which(sex_height_g$p_fdr < 0.1)


height_sex_g <- corncob::differentialTest(formula = ~ Extraction_Number + A_SDC_GENDER + PM_STANDING_HEIGHT_AVG,
                          formula_null = ~ Extraction_Number + PM_STANDING_HEIGHT_AVG,
                          phi.formula = ~ Extraction_Number + A_SDC_GENDER + PM_STANDING_HEIGHT_AVG,
                          phi.formula_null = ~ Extraction_Number + A_SDC_GENDER + PM_STANDING_HEIGHT_AVG,
                          data=phylo_comp_g, test="Wald", fdr_cutoff = 0.1, boot=F)

height_sex_g$significant_taxa


sex_ffm_g <- corncob::differentialTest(formula = ~ Extraction_Number + A_SDC_GENDER + PM_BIOIMPED_FFM,
                          formula_null = ~ Extraction_Number + A_SDC_GENDER,
                          phi.formula = ~ Extraction_Number + A_SDC_GENDER + PM_BIOIMPED_FFM,
                          phi.formula_null = ~ Extraction_Number + A_SDC_GENDER + PM_BIOIMPED_FFM,
                          data=phylo_comp_g, test="Wald", fdr_cutoff = 0.1, boot=F)

plot(sex_ffm_g)
sex_ffm_g$significant_models


ffm_sex_g <- corncob::differentialTest(formula = ~ Extraction_Number + A_SDC_GENDER + PM_BIOIMPED_FFM,
                          formula_null = ~ Extraction_Number + PM_BIOIMPED_FFM,
                          phi.formula = ~ Extraction_Number + A_SDC_GENDER + PM_BIOIMPED_FFM,
                          phi.formula_null = ~ Extraction_Number + A_SDC_GENDER + PM_BIOIMPED_FFM,
                          data=phylo_comp_g, test="Wald", fdr_cutoff = 0.1, boot=F)

ffm_sex_g$significant_taxa


sex_height_a <- corncob::differentialTest(formula = ~ Extraction_Number + A_SDC_GENDER + PM_STANDING_HEIGHT_AVG,
                          formula_null = ~ Extraction_Number + A_SDC_GENDER,
                          phi.formula = ~ Extraction_Number + A_SDC_GENDER + PM_STANDING_HEIGHT_AVG,
                          phi.formula_null = ~ Extraction_Number + A_SDC_GENDER + PM_STANDING_HEIGHT_AVG,
                          data=phylo_comp, test="Wald", fdr_cutoff = 0.1, boot=F)

plot(sex_height_a)
sex_height_a$significant_models

height_sex_a <- corncob::differentialTest(formula = ~ Extraction_Number + A_SDC_GENDER + PM_STANDING_HEIGHT_AVG,
                          formula_null = ~ Extraction_Number + PM_STANDING_HEIGHT_AVG,
                          phi.formula = ~ Extraction_Number + A_SDC_GENDER + PM_STANDING_HEIGHT_AVG,
                          phi.formula_null = ~ Extraction_Number + A_SDC_GENDER + PM_STANDING_HEIGHT_AVG,
                          data=phylo_comp, test="Wald", fdr_cutoff = 0.1, boot=F)

height_sex_a$significant_taxa

sex_ffm_a <- corncob::differentialTest(formula = ~ Extraction_Number + A_SDC_GENDER + PM_BIOIMPED_FFM,
                          formula_null = ~ Extraction_Number + A_SDC_GENDER,
                          phi.formula = ~ Extraction_Number + A_SDC_GENDER + PM_BIOIMPED_FFM,
                          phi.formula_null = ~ Extraction_Number + A_SDC_GENDER + PM_BIOIMPED_FFM,
                          data=phylo_comp, test="Wald", fdr_cutoff = 0.1, boot=F)
...
sex_ffm_a$significant_models

ffm_sex_a <- corncob::differentialTest(formula = ~ Extraction_Number + A_SDC_GENDER + PM_BIOIMPED_FFM,
                          formula_null = ~ Extraction_Number + PM_BIOIMPED_FFM,
                          phi.formula = ~ Extraction_Number + A_SDC_GENDER + PM_BIOIMPED_FFM,
                          phi.formula_null = ~ Extraction_Number + A_SDC_GENDER + PM_BIOIMPED_FFM,
                          data=phylo_comp, test="Wald", fdr_cutoff = 0.1, boot=F)
...
ffm_sex_a$significant_taxa



ffm_height_sex_g <- corncob::differentialTest(formula= ~ Extraction_Number + PM_BIOIMPED_FFM + PM_STANDING_HEIGHT_AVG + A_SDC_GENDER,
                                            formula_null = ~ Extraction_Number + PM_BIOIMPED_FFM + PM_STANDING_HEIGHT_AVG,
                                            phi.formula = ~ Extraction_Number + PM_BIOIMPED_FFM + PM_STANDING_HEIGHT_AVG + A_SDC_GENDER,
                                            phi.formula_null = ~ Extraction_Number + PM_BIOIMPED_FFM + PM_STANDING_HEIGHT_AVG + A_SDC_GENDER,
                                            data=phylo_comp_g, test="Wald", fdr_cutoff = 0.1, boot = F)

ffm_height_sex_g$significant_taxa


ffm_height_sex_a <- corncob::differentialTest(formula= ~ Extraction_Number + PM_BIOIMPED_FFM + PM_STANDING_HEIGHT_AVG + A_SDC_GENDER,
                                            formula_null = ~ Extraction_Number + PM_BIOIMPED_FFM + PM_STANDING_HEIGHT_AVG,
                                            phi.formula = ~ Extraction_Number + PM_BIOIMPED_FFM + PM_STANDING_HEIGHT_AVG + A_SDC_GENDER,
                                            phi.formula_null = ~ Extraction_Number + PM_BIOIMPED_FFM + PM_STANDING_HEIGHT_AVG + A_SDC_GENDER,
                                            data=phylo_comp, test="Wald", fdr_cutoff = 0.1, boot = F)

ffm_height_sex_a$significant_taxa
```


```{r, eval=T, fig.fullwidth=TRUE, fig.width=10, fig.height=10, Analysis_of_results}
...
breaks=seq(-2, 2, length.out = 400) #41 values
#now add outliers
...
rownames(set_na_filt_ACorn_cob_coef)[1] <- "ASV-0c2af Treponema 2 subsp. socranskii"
...
rownames(set_na_filt_ACorn_cob_coef)[36] <- "ASV-ca12e Streptococcus subsp. anginosus"
...
ASV_layout_mat <- rbind(c(4,0,0), c(2,1,0), c(0,3,0))
ASV_lwid=c(2,3.5,.00001)
ASV_lhei=c(.2,3,.3)


breaks=seq(-2, 2, length.out = 300) #41 values
#now add outliers
breaks=append(breaks, 10000)
breaks=append(breaks, -10000, 0)
#create colour panel with length(breaks)-1 colours
...
ASV_heat <- heatmap.2(as.matrix(reorder_set_na_filt_ACorn_cob_coef),na.color="black", density.info = "none", symkey = F, trace="none", srtCol=35, dendrogram = "none", key.xlab = "log odds", key.ylab = "p Value", key=T, col=mycol, lmat = ASV_layout_mat, lwid = ASV_lwid, lhei = ASV_lhei, cexRow = 1.3, cexCol = 1.3, Colv=F, distfun=function(x) dist(x, method="manhattan"), colRow = taxa_colors, Rowv=F, offsetRow = -58.5, sepcolor = "black", rowsep = c(7, 20, 35, 37,  41), keysize=0.2, key.par = list(cex=0.01, cex.axis=60, cex.main=60, mar=c(70,50,50,50), mgp=c(.01,50,1), tcl=-10))
