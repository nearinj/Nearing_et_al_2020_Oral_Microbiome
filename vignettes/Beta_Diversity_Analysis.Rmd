...
set.seed(1995)
sex_height_control <- adonis2(W_unifrac[["Distance"]] ~ W_unifrac[["Metadata"]]$Extraction_Number + W_unifrac[["Metadata"]]$A_SDC_GENDER +
                                W_unifrac[["Metadata"]]$PM_STANDING_HEIGHT_AVG, parallel = 40, by="margin", permutations = 1000)


sex_height_control

set.seed(1995)
sex_ffm_control <- adonis2(W_unifrac[["Distance"]] ~ W_unifrac[["Metadata"]]$Extraction_Number + W_unifrac[["Metadata"]]$A_SDC_GENDER +
                                W_unifrac[["Metadata"]]$PM_BIOIMPED_FFM, parallel = 40, by="margin", permutations = 1000)
...
set.seed(1995)
sex_whr_control <- adonis2(W_unifrac[["Distance"]] ~ W_unifrac[["Metadata"]]$Extraction_Number + W_unifrac[["Metadata"]]$A_SDC_GENDER +
                                W_unifrac[["Metadata"]]$PM_WAIST_HIP_RATIO, parallel = 40, by="margin", permutations = 1000)
...
effect_size_plot <- ggplot(table2, aes(x=Feature, y=R2, fill=color)) + geom_bar(stat="identity") + coord_flip() +
  ...
  guides(fill=guide_legend(title="Type")) + scale_fill_identity(guide="legend", labels=c("Diet", "Life Style", "Sex", "Anthropometric", "Age")) + scale_y_continuous(labels = scaleFUN) + ylim(c(0,0.01))
...
library(ggrepel)
weighted_uni_gg_bi <- ggplot(data = plot_data_weight, aes(PC1, PC2)) + geom_point(colour="black", alpha=0.2) + coord_fixed() +
...
library(ggrepel)
bray_curt_gg_bi <- ggplot(data = bray_plot_data_weight, aes(PC1, PC2)) + geom_point(colour="black", alpha=0.2) + coord_fixed() +
